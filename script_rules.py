...
pattern_standssl = re.compile(r"access-list (?P<name_rule>\S+) (?=.*\bstandard\b).+permit (?P<srcaddr>\d[0-9].*)")
pattern_acc_std = re.compile(r"access-list (?P<name_rule>\S+) extended permit ip (?P<srcaddr>\S.+) (?P<dstaddr>any)")

# Retrieve data access-list extended IP
nmmatch_any = re.findall(pattern_any, cmd)
nmmatch_acc_std = re.findall(pattern_acc_std, cmd)
...
tcp_pattern = re.compile(r"access-list (?P<name_rule>\S+).+tcp (?P<svc>\w+) object-group (?P<src>\S+) object-group (?P<dst>\S+)")
tcp_patternip = re.compile(r"access-list (?P<name_rule>\S+).+tcp (?P<svc>\S+) (?P<srcaddr>\d.{1,3}.{1,3}.{1,3}.{1,3} .{1,3}.{1,3}.{1,3}.{1,3}.{1,3}) (\w+.\w+ )(?P<dstaddr>\S+)")
...
denypattern = re.compile(r"access-list (?P<name_rule>\S+).+ deny (\w+.\w+) (?P<service>\S+) (?P<srcaddr>\S+) (\w+.\w+) (?P<dstaddr>\S+)")
